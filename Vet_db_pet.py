...
class Pet:
    ...
    def read_pet_data(self):
        ...
        self.breed = input("Enter Pet Bread: ")
    ...
    def get_insert_sql_query(self):
        sql = "insert into Pet values(null, '{name}', {age}, {weight}, '{breed}', " \
              "'{gender}', {cid}, '{createdon}');".format_map(vars(self))
        return sql
    ...
    def get_update_sql_query(self):
        sql = "update Pet set name='{name}', age={age}, weight={weight}, breed='{breed}', gender='{gender}' " \
              "where pid = {pid}".format_map(vars(self))
        return sql
